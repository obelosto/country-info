...
async function getLand(land) {
    try {
        ...
        const result = await (0, _axiosDefault.default).get(uri);
        console.log(result.data[0]);
        const { name , subregion , population , flag , capital , currencies  } = result.data[0];
        let currenciesText = "";
        if (currencies.length > 1) {
            for(let i = 0; i < currencies.length; i++)if (i === currencies.length - 1) currenciesText += currencies[i].name + " currencies";
            else currenciesText += currencies[i].name + ", ";
        } else currenciesText = currencies[0].name + " currency";
        const landLine = document.createElement("p");
        landLine.innerHTML = `<article>
    ...
    /*function countCurrencies(){
        ...
        console.log(currenciesText + " valuti");*/ } catch (e) {
        console.error(e);
...
function inputCompleteCheck(e) {
    ...
    } else passAlertBox.textContent = "Het search-veld is te kort, gebruik minimaal 3 tekens!";
}
